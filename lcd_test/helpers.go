...
import (
	...
	gapp "github.com/bhpnet/bhp-dev/app"
)
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewBhpApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.BhpApp,
) (*nm.Node, error) {
...
// NOTE: If making updates here also update cmd/bhp/cmd/bhpcli/main.go
