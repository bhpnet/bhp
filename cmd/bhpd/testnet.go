...
func testnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Bhpd testnet",
...
Example:
	bhpd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "bhpd",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "bhpcli",
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("2.5%s", sdk.DefaultBondDenom),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 2.5abhp,0.001stake)")
	return cmd
}
...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	bhpConfig := srvconfig.DefaultConfig()
	bhpConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		//accTokens := sdk.TokensFromConsensusPower(1000*100000000)
		//accStakingTokens := sdk.TokensFromConsensusPower(500)
		accStakingTokens := sdk.TokensFromConsensusPower(1250 * 10000)
		...
		valTokens := sdk.TokensFromConsensusPower(200 * 10000)
		...
		// TODO: Rename config file to server.toml as it's not particular to Bhp
		// (REF: https://github.com/cosmos/cosmos-sdk/issues/4125).
		bhpConfigFilePath := filepath.Join(nodeDir, "config/bhpd.toml")
		srvconfig.WriteConfigFile(bhpConfigFilePath, bhpConfig)
	}
