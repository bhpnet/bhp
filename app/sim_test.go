...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *BhpApp) (
...
func testAndRunTxs(app *BhpApp) []simulation.WeightedOperation {
	cdc := MakeCodec()
...
func invariants(app *BhpApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/BhpApp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := NewBhpApp(logger, db, nil, true, 0)
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewBhpApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "BhpApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewBhpApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	newApp := NewBhpApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "BhpApp", newApp.Name())
